...
```
Well done! You have just accomplished all the necessary steps needed to run the application on you local machine.

## Run the TensorFlow stack
### Build & push
Now that we are capable of deploying the TensorFlow stack on localhost, let's clone [fullstack-tf](https://github.com/ariellev/fullstack-tensor-flow.git) and proceed with the following commands
```bash
...
Step 1/5 : FROM frolvlad/alpine-miniconda3:python3.6
 ...
 ..
```
The `push-all-docker.sh` script scans the working directory recursively for any Dockerfile. Any match is being built and subsequently pushed to a given registry. The script takes several minutes to complete and it produces a fair amount of output. At the end you can expect to see 4 new images created:
```bash
...
Out script created 4 new images:

* **tensor-jupyter**: Running a **JupterLab** instance, which is *"An extensible environment for interactive and reproducible computing"*<sup>[1](#jupyterlab)</sup>. You will use this web based IDE to run the code for training and exporting the **MNIST** neural network.
...
```
Let's return to our litte [Swagger UI](http://localhost:8080/tf/#!/mnist/post_mnist). This time instead of `mnist-net` let's type in `mnist-tutorial` into the model value input field. When you next click on `Try it out!` you'll see a response with the same payload as before, nevertheless the data has originated this time from the model you had freshly deployed.
...
### Nginx
Flask is not meant under any circumstances to be "client facing", as it does not scale well and by default serves only one request at a time. According to the official [documentation](http://flask.pocoo.org/docs/1.0/tutorial/deploy/#run-with-a-production-server):
...
## Beyond localhost
The container orchestration market is highly saturated<sup>[3](#docker-compose)</sup> meaning at the very least, that the number of well established products we had to theoretically cover is unfortunately above and beyond any feasible scope. Still dockerizing an application and running it locally, also when not exactly the final destination, is yet a major step forward in achieving the goal of deploying the application to a productive environment. So **way to go!** you are now mastering an important skill.

And regarding the future.. well..  
*Roll your sleeves up! Nail the services to production and Good Luck!*

![alt Cloud](https://raw.githubusercontent.com/ariellev/fullstack-tensor-flow/master/images/blah-blah-cloud.png)

___
<a name="jupyterlab" href="https://github.com/jupyterlab/jupyterlab">1. Jupyter Lab</a>  
<a name="docker-compose" href="https://docs.docker.com/compose/">2. Docker Compose</a>  
<a name="container-adoption-survey" href="https://portworx.com/2017-container-adoption-survey/">3. 2017 Container Adoption Survey</a>
